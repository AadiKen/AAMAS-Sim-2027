...
class Sonar:
    """Idealized downward bathymetric range sonar; no acoustic propagation model."""
    kind = "physical"
    requirements = frozenset({"geometry.raycast", "bathymetry.bottom_z"})
    ...
    def sample(self, context: SensorContext, *, sample_step: int) -> SensorPacket:
        ...
        try:
            ...
            raise ExternalDataUnavailableError("Sonar requires canonical bathymetry") from exc
        ...
        for index in range(self.beam_count):
            ...
            result = environment.sample("geometry.raycast", mount.position_ned_m,
                ...
                max_range_m=self.max_range_m, include_entities=False).value
            distance = result["distance_m"] if result["hit"] else None
