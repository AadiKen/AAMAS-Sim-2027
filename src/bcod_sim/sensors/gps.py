...
class GPS:
    ...
    def __init__(self, config: SensorConfig, *, origin_wgs84_rad_m: tuple[float, float, float],
                 error_model: GPSErrorModel | None = None) -> None:
        ...
        self.error_model = error_model or GPSErrorModel(config.noise_std, config.noise_std)

    def sample(self, context: SensorContext, *, sample_step: int) -> SensorPacket:
        mount = mount_kinematics(self.config, context)
        position = (mount.position_ned_m + mount.position_ned_m.new_tensor(self.error_model.position_bias_ned_m) +
                    stable_noise(self.config, sample_step, (3,), dtype=mount.position_ned_m.dtype,
                                 device=mount.position_ned_m.device, stream="position",
                                 std=self.error_model.position_noise_std_m))
        lat, lon, altitude = ned_to_geodetic(tuple(position.tolist()), self.origin)
        velocity = (mount.velocity_ned_mps + position.new_tensor(self.error_model.velocity_bias_ned_mps) +
                    stable_noise(self.config, sample_step, (3,), dtype=position.dtype,
                                 device=position.device, stream="velocity",
                                 std=self.error_model.velocity_noise_std_mps))
