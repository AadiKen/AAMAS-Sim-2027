...
def test_smoke_geometry_is_deterministic_closed_and_dimensioned(tmp_path):
    ...
    first=import_ascii_stl(a,tmp_path/"a-normalized.stl",source_frame="FPU_m")
    second=import_ascii_stl(b,tmp_path/"b-normalized.stl",source_frame="FPU_m")
    assert first.content_hash==second.content_hash
    assert np.allclose(first.bounds_m,((-1,-.2,-.075),(1,.2,.075)))
    with pytest.raises(CFDExecutionError):
        import_ascii_stl(Path(__file__),tmp_path/"bad.stl",source_frame="FPU_m")


def test_production_case_generator_has_real_mesh_solver_and_force_contract(tmp_path):
    source=generate(tmp_path/"source.stl")
    geometry=import_ascii_stl(source,tmp_path/"normalized.stl",source_frame="FPU_m")
